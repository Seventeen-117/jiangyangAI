...
public class SsoServiceImpl implements SsoService {
    ...
    @Override
    public String buildAuthorizeUrl(String redirectUri, String state, ServerWebExchange exchange) {
    ...
    @Override
    public SsoAuthResponse handleCallback(String code, String state, ServerWebExchange exchange) {
    ...
    @Override
    public void logout(String accessToken, ServerWebExchange exchange) {
    ...
    @Override
    public void setAuthCookies(ServerWebExchange exchange, SsoAuthResponse authResponse) {
        // 设置访问令牌Cookie
        ResponseCookie accessTokenCookie = ResponseCookie.from("access_token", authResponse.getAccessToken())
                .path("/")
                .httpOnly(true)
                .secure(false) // 生产环境应该设置为true
                .maxAge(authResponse.getExpiresIn().intValue())
                .build();
    ...
    @Override
    public Map<String, Object> getSessionStatus(ServerWebExchange exchange) {
        Map<String, Object> sessionInfo = new HashMap<>();
        
        // 从Cookie中获取令牌
        ResponseCookie accessTokenCookie = (ResponseCookie) exchange.getRequest().getCookies().getFirst("access_token");
        ResponseCookie refreshTokenCookie = (ResponseCookie) exchange.getRequest().getCookies().getFirst("refresh_token");
    ...
    private void clearSession(ServerWebExchange exchange) {
        // 清除HTTP会话
        exchange.getSession()
            .filter(session -> session != null)
            .subscribe(session -> session.invalidate());
    }
