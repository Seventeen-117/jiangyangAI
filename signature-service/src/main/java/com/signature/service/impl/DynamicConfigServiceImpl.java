...
public class DynamicConfigServiceImpl implements DynamicConfigService {
    ...
    @Override
    public boolean isAsyncValidationEnabled() {
        return isValidationRuleEnabled("ASYNC_VALIDATION");
    }

    @Override
    public boolean isAsyncValidationExcludedPath(String path, String httpMethod) {
        try {
            List<String> excludedPaths = getExcludedPaths(httpMethod);
            boolean isExcluded = PathMatcherUtil.matchesAny(path, excludedPaths.toArray(new String[0]));
            log.debug("Async validation path exclusion check: path={}, httpMethod={}, isExcluded={}", 
                     path, httpMethod, isExcluded);
            return isExcluded;
        } catch (Exception e) {
            log.error("Error checking async validation excluded path: path={}, httpMethod={}", 
                     path, httpMethod, e);
            return false;
        }
    }
